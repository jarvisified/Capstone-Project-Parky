...
✨ Key Features
Real-time Data Pipeline: Powered by Pathway, ensuring continuous processing of incoming parking data.
...
Interactive Visualizations: Live dashboards built with Bokeh to monitor price evolution, utilization, and demand patterns.
...
Data Ingestion: Raw parking data, including occupancy, queue length, and environmental factors, is ingested as a continuous stream.

Feature Engineering: Pathway processes this raw data, extracting and transforming features relevant for pricing decisions.

Dynamic Pricing Logic: Custom-built models (using NumPy and Pandas) apply economic principles and machine learning to predict optimal prices based on the processed features.

Real-time Price Output: The calculated prices are continuously emitted for each parking lot.

Analysis & Visualization: Key metrics like utilization, occupancy, and queue lengths are analyzed. All outputs are fed into Bokeh to create interactive, real-time dashboards that visually justify pricing behavior and show performance trends.
...
Open the Notebook: Open ParkSense.ipynb (or Parky (1).ipynb if that's the primary notebook) in Google Colab.

Run Cells: Execute the cells sequentially. The final cell containing pw.run() will start the real-time data pipeline and update the Bokeh plots continuously.
...
├── parking_data_stream.csv   # (Optional) Simulated real-time data stream
